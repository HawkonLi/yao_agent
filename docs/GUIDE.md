...
> 面向 **Python 开发者**。用 Python 的生成器、dataclass、描述符，实现声明式智能体编排。

---

## 0. 名字与哲学

**爻（yáo）**是八卦的基本单元。阴爻（⚋）阳爻（⚊）的不同组合，生出八卦；八卦两两相重，演为六十四卦。

YaoAgent 同理：指令、工具、钩子就是你的"爻"，层层组合出各种智能体。
它不是写死一段 prompt 字符串，而是**声明**"这个状态下，模型该看到哪些指令、哪些工具、用什么参数"——
框架在每次请求前重新求值这个声明。组合变，行为就变。
